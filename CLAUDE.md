...
- **名前**: `growi-plugin-test2`
- **種別**: GROWI Script プラグイン (`schemaVersion: 4`, `types: [<<...>>]`)
- **目的**: アラートブロックを表示する
- **拡張記法 / UI**: 下記のような5種類
...
`window.pluginActivators[grow-plugin-test2] = { activate, deactivate }` を登録する。GROWI はプラグイン有効化時に `activate()` を呼ぶ。
...
### 7. GROWI 内蔵の `--callout-accent-*` CSS 変数との干渉

GROWI (v7 系の一部バージョン) は `blockquote` 内の `[!TYPE]` 記法を内部で検出し、`--callout-accent-{type}` (`note` / `tip` / `important` / `warning` / `caution`) という CSS 変数で配色する独自実装を持つ。GitHub Alerts 風プラグインを実装すると、GROWI 内蔵スタイルとプラグインのスタイルの両方が適用され、以下が起こりうる:

- **IMPORTANT が紫ではなく青で表示される** (GROWI 内蔵の `--callout-accent-important` が NOTE 同等の青になっているケースを実機で確認)
- アイコンやタイトル文字の周辺に黒い枠が残る (内蔵 callout の枠線)
- DevTools の Computed タブで blockquote 親に `--callout-accent-{type}` が表示されていれば内蔵 callout が動作している証拠

対策の優先順:

1. **CSS セレクタの specificity を上げる**: `.github-alert.github-alert-important` のように 2 クラス組み合わせて (0,2,0) にし、GROWI 内蔵のセレクタに勝つ
2. **`--callout-accent-*` 自体を再定義**: プラグイン CSS で GROWI の変数を上書きすると、内蔵 callout の配色も合わせて正しい色に揃えられる
   ```css
   .github-alert.github-alert-important {
     --callout-accent-important: #8250df;
     --gha-color: #8250df;
   }
   ```
3. **`border: 0` で枠線リセット → `border-left` のみ再宣言**: 黒い囲み対策
4. **GROWI の dark mode**: `prefers-color-scheme: dark` だけでは追随しないことがある。`[data-bs-theme="dark"]` セレクタを併用すると Bootstrap 5 ベースの GROWI テーマ切替に確実に追随する
5. **最終手段として `!important`**: 上記でも勝てない場合のみ
